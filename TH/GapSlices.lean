...
The two slices of the kernel (K) that the Corvaja–Zannier Main Theorem
(`CZ.pseudoPisot_approx_of_subspace`, [CZ04] — **derived** from the Subspace
Theorem in `CITED/CorvajaZannierProof.lean`) kills:
...
* **2b′, huge gaps — Theorem B2** (`hugeGap_slice_finite`): for every `θ` there
  ...
  `ε′ < (3/4)·log θ⁻¹ / (log 2 + 2 log 3)` guarantee.
...
`TH.CapstoneM4`, and discharged as Stage 2c in `TH.GapDichotomy`.
...
/-- 2b′ window nesting: with `ε = min 1 (log θ⁻¹/(4 log 3))` and
...
/-- **Stage 2b**: for every fixed gap `s₀ ≥ 1` and every rational
...
/-- **Bounded-gap repetitions are short** (corollary of Stage 2b): for every
...
`D = 0` subspace degeneracy for good.  Ineffective; footprint std3 + [CZ04]. -/
...
/-- **Stage 2b′ / Theorem B2**: for every rational scale
